...
#include <c:/programacion/soulsR/sprites.bas>
#include <c:/programacion/soulsR/mapas.bas>
:rem #PRAGMA org=24000

DIM iniUDGS,ani,anisube,anienem,frames,t,contestamina,topcontestamina AS UINTEGER
...
0,0,1,0,2,2,2,2,2,0} :rem ------------------------- tabla donde hay enemigos    
...
DIM enemigo,contani,topecontani,contanienem,topecontanienem,pant,ataca,atacaenem,retirada,parado,escudo,energia,magia,estamina,topenergia,nivel,souls,llaves,lado,c1,c2,c3,c4,c5,c6,c7,c8,c9 AS UBYTE
DIM energiaenem,energiatmp,energiajefe,quien,xx,yy,cae,sube,tile,xcur,ycur,yc,xc,xh,yh,panth,pantsouls,soulstmp,s,contdisparo,contbola,disparando,xb,yb,xxb,contb,tipo,magiamago AS UBYTE
...
quien=0                  :rem quien quita energia (1=caballero 0=enemigo)
...
IF contestamina>=topcontestamina THEN contestamina=0
...
IF souls>=17 THEN 
   ...
   IF topecontani>2 THEN topecontani=topecontani-1:END IF
   IF topenergia<12 THEN topenergia=topenergia+1:END IF
   energia=topenergia
   pintaener()
...
IF pant=9 THEN 
   jefe()
   IF energiajefe=0 THEN fin():END IF
END IF
...
IF cae=0 and sube=0 and mapa(pant,(y+3)/3,x/3)<20 and mapa(pant,(y+3)/3,(x+1)/3)<20 and mapa(pant,(y+3)/3,(x+2)/3)<20 THEN cae=1:END IF:rem ---- se cae
IF cae=1 THEN 
   borraprota(x,y,3)
   y=y+1
   ...
   END IF
   ink 7
   IF dir=1 THEN POKE UINTEGER iniUDGS,@caballero3(0):END IF
   IF dir=-1 THEN POKE UINTEGER iniUDGS,@caballero3(72):END IF
   imprime()
...
IF ataca=0 THEN
IF IN 57342=254 and cae=0 and sube=0 or IN 57342=190 and cae=0 and sube=0 THEN :rem --- tecla P
   contani=contani+1
   ...
   IF mapa(pant,y/3,(x+3)/3)=38 THEN :rem --------------------  aprende el hechizo
      magiamago=1
      IF magia=0 THEN magia=magia+3:END IF
      pintaener()
   ...
   IF contani>=topecontani and mapa(pant,y/3,(x+3)/3)<20 THEN :rem --------------------- se mueve     
     ...
     ani=ani+72
     borraprota(x,y,1)
     IF ani=432 THEN ani=0:END IF
     IF ani=216 or ani=0 THEN beep 0.0025,-20:END IF
     IF enemigo=1  THEN:rem -------------------------------------detecta el choque con el enemigo según el tipo por la derecha
       IF tipo=1 THEN
          IF y<>yesq THEN x=x+1
          ELSE 
             IF xesq<>x+3 THEN x=x+1:END IF
          END IF         
       END IF
       IF tipo=2 THEN
   ...
   END IF
ELSE
   IF IN 57342=253 and cae=0 and sube=0 or IN 57342=189 and cae=0 and sube=0 THEN :rem --- tecla O
     contani=contani+1
     contestamina=contestamina+1
     dir=-1
     parado=0
     ...
     IF contani>=topecontani and mapa(pant,y/3,(x-1)/3)<20 THEN :rem -------------------- se mueve
      ...
      IF enemigo=1 THEN :rem --------------------------------- detecta el choque con el enemigo según el tipo por la izquierda
        IF tipo=1 THEN
           IF y<>yesq THEN x=x-1
           ELSE
              IF xesq+3<>x THEN x=x-1:END IF
           END IF
        END IF
        IF tipo=2 THEN
           IF y<>yesq+3 THEN x=x-1
   ...
   ELSE :rem ---------------------------------------------------------- parado
     ...
     END IF      
     IF sube=0 THEN contani=0:END IF
     ani=0
     ink 7
...
IF IN 64510=254 and ataca=0 and parado=1 OR IN 64510=190 and ataca=0 and parado=1 THEN :rem ----- TECLA Q ---------------- se protege y mira si puede subir

  IF mapa(pant,y/3,x/3)=1 and mapa(pant,y/3,(x+1)/3)=1 and mapa(pant,y/3,(x+2)/3)=1 and sube=0 THEN :rem ----- mira si puede subir escaleras
      sube=1:contani=0
  ...
  IF sube=1 THEN contani=contani+1
     IF contani>=topecontani THEN contani=0
        IF mapa(pant,(y+2)/3,x/3)=1 or mapa(pant,(y+2)/3,x/3)=21 THEN
           ...
           END IF
           anisube=anisube+72
           IF anisube=144 THEN anisube=0:END IF
           POKE UINTEGER iniUDGS,@caballerosube(anisube)
           ink 7
           imprime() 
           beep 0.001,70         
  ...
  IF sube=0 THEN escudo=1 :rem ------------------------ se protege con el escudo
     IF dir=1  THEN ani=0:END IF
     IF dir=-1 THEN ani=72:END IF
...
IF IN 65022=254 and ataca=0 OR IN 65022=190 and ataca=0 THEN :rem ---------- TECLA A ----------------------------- mira si puede bajar
...
IF IN 32766=190 and ataca=0 and sube=0 OR IN 32766=254 and ataca=0 and sube=0 THEN ataca=1:ani=0:contani=0:END IF :rem ----- TECLA SPACE ----- ataca

IF ataca=1 THEN contani=contani+1
  IF contani>=topecontani THEN contani=0
    IF ani=432 THEN ani=0:ataca=0:contestamina=contestamina+50:END IF
    IF dir=1  THEN POKE UINTEGER iniUDGS,@caballeroatt(ani):END IF
    IF dir=-1 THEN POKE UINTEGER iniUDGS,@caballeroatt2(ani):END IF
    ink 7
    imprime()
    IF ani=144 THEN beep 0.0025,-20:END IF
    IF ani=216 THEN 
        ink 7 
        IF dir=1 and x<21 THEN POKE UINTEGER iniUDGS,@espada(0)  :print at y+1,x+3;"\a"
            IF enemigo=1 THEN
                 IF tipo=1 and xesq=x+3 and y=yesq THEN beep 0.001,-20:beep 0.001,0:beep 0.001,30:restapone(1):energiaenem=energiaenem-1:END IF
                 IF tipo=2 and xesq=x+3 and y=yesq+3 THEN beep 0.001,-20:beep 0.001,0:beep 0.001,30:restapone(1):energiaenem=energiaenem-1:END IF
                 IF tipo=3 and xesq=x+3 and y=yesq-1 THEN beep 0.001,-20:beep 0.001,0:beep 0.001,30:restapone(1):energiaenem=energiaenem-1:END IF
            END IF
...
IF IN 32766=251 and magiamago=1 or IN 32766=187 and magiamago=1 THEN : rem ----------- TECLA M -------------------------------- lanza la magia
  IF magia>0 THEN
    ...
    IF mapa(pant,ycur/3,(xcur/3)+1)=39 THEN 
       ...
       fxm()
    END IF
    IF pant=9 THEN energiajefe=energiajefe-1:END IF   
  END IF
END IF
end sub
...
IF xesq>x and yesq=y and abs(xesq-x)<=9 and atacaenem=0 and retirada=0 THEN contanienem=contanienem+1:direnem=-1 :rem --- se acerca si esta caballero cerca
    IF contanienem>=topecontanienem+4 THEN 
      contanienem=0
      IF xesq>x+3 and mapa(pant,(yesq+3)/3,(xesq-1)/3)<>0 and mapa(pant,yesq/3,(xesq-1)/3)=0 THEN borraesq(xesq,yesq):xesq=xesq-1:END IF
...
IF xesq<x and yesq=y and abs(xesq-x)<=9 and atacaenem=0 and retirada=0 THEN contanienem=contanienem+1:direnem=1 :rem --- se acerca si esta caballero cerca
    IF contanienem>=topecontanienem+4 THEN 
      contanienem=0
      IF xesq+3<x and mapa(pant,(yesq+3)/3,(xesq+3)/3)<>0 and mapa(pant,yesq/3,(xesq+3)/3)=0 THEN borraesq(xesq,yesq):xesq=xesq+1:END IF
...
IF tipo=3 THEN
  ...
  IF abs(xesq-x)<12 and y=yesq-1 THEN contanienem=contanienem+1:atacaenem=1
     ...
     END IF 
  ELSE
    IF atacaenem=1 THEN ataenem=0:anienem=0:ink 5
...
sub restapone(quien) :rem -------------------------------- PONE O QUITA ENERGIA ------------------------------------------------------------------------
...
sub borraprota(x,y,lado):rem ----------------------------- PARA BORRAR AL CABALLERO ---------------------------------------------------------------------
   ...
   IF lado=1 THEN
      IF xcur<=x THEN
         IF abs(x-xcur)=0 THEN print at ycur,xcur;ink c1;"\a":print at ycur+1,xcur;ink c4;"\d":print at ycur+2,xcur;ink c7;"\g":END IF
         IF abs(x-xcur)=1 THEN print at ycur,xcur+1;ink c2;"\b":print at ycur+1,xcur+1;ink c5;"\e":print at ycur+2,xcur+1;ink c8;"\h":END IF
         IF abs(x-xcur)=2 THEN print at ycur,xcur+2;ink c3;"\c":print at ycur+1,xcur+2;ink c6;"\f":print at ycur+2,xcur+2;ink c9;"\i":END IF
      ELSE
         tile=mapa(pant,ycur/3,(xcur/3)-1)
         lista()
         IF abs(x-xcur)=1 THEN print at ycur,xcur-1;ink c3;"\c":print at ycur+1,xcur-1;ink c6;"\f":print at ycur+2,xcur-1;ink c9;"\i":END IF
         IF abs(x-xcur)=2 THEN print at ycur,xcur-2;ink c2;"\b":print at ycur+1,xcur-2;ink c5;"\e":print at ycur+2,xcur-2;ink c8;"\h":END IF
      END IF   
   END IF
   
   IF lado=2 THEN
      IF xcur>=x THEN
         IF abs(x-xcur)=0 THEN print at ycur,xcur+2;ink c3;"\c":print at ycur+1,xcur+2;ink c6;"\f":print at ycur+2,xcur+2;ink c9;"\i":END IF
         IF abs(x-xcur)=1 THEN print at ycur,xcur+1;ink c2;"\b":print at ycur+1,xcur+1;ink c5;"\e":print at ycur+2,xcur+1;ink c8;"\h":END IF
         IF abs(x-xcur)=2 THEN print at ycur,xcur;ink c1;"\a":print at ycur+1,xcur;ink c4;"\d":print at ycur+2,xcur;ink c7;"\g":END IF
      ELSE
         tile=mapa(pant,ycur/3,(xcur/3)+1)
         lista()
         IF abs(x-xcur)=1 THEN print at ycur,xcur+3;ink c1;"\a":print at ycur+1,xcur+3;ink c4;"\d":print at ycur+2,xcur+3;ink c7;"\g":END IF
         IF abs(x-xcur)=2 THEN print at ycur,xcur+4;ink c2;"\b":print at ycur+1,xcur+4;ink c5;"\e":print at ycur+2,xcur+4;ink c8;"\h":END IF
      END IF   
   END IF
   IF lado=4 THEN
      IF ycur>=y THEN   
         IF abs(y-ycur)=0 THEN print at ycur+2,xcur;ink c7;"\g";ink c8;"\h";ink c9;"\i":END IF
         IF abs(y-ycur)=1 THEN print at ycur+1,xcur;ink c4;"\d";ink c5;"\e";ink c6;"\f":END IF
         IF abs(y-ycur)=2 THEN print at ycur,xcur;ink c1;"\a";ink c2;"\b";ink c3;"\c":END IF
      ELSE
         tile=mapa(pant,(ycur/3)+1,xcur/3)
         lista()
         IF abs(y-ycur)=1 THEN print at ycur+3,xcur;ink c1;"\a";ink c2;"\b";ink c3;"\c":END IF
         IF abs(y-ycur)=2 THEN print at ycur+4,xcur;ink c4;"\d";ink c5;"\e";ink c6;"\f":END IF
      END IF   
   END IF
   IF lado=3 THEN
      IF ycur<=y THEN
         IF abs(y-ycur)=0 THEN print at ycur,xcur;ink c1;"\a";ink c2;"\b";ink c3;"\c":END IF
         IF abs(y-ycur)=1 THEN print at ycur+1,xcur;ink c4;"\d";ink c5;"\e";ink c6;"\f":END IF
         IF abs(y-ycur)=2 THEN print at ycur+2,xcur;ink c7;"\g";ink c8;"\h";ink c9;"\i":END IF
      ELSE
         tile=mapa(pant,(ycur/3)-1,xcur/3)
         lista()
         IF abs(y-ycur)=1 THEN print at ycur-1,xcur;ink c7;"\g";ink c8;"\h";ink c9;"\i":END IF
         IF abs(y-ycur)=2 THEN print at ycur-2,xcur;ink c4;"\d";ink c5;"\e";ink c6;"\f":END IF
      END IF  
   END IF
...
sub borraesq(xesq,yesq) :rem ------------------------ PARA BORRAR AL ESQUELETO ---------------------------------------------------------------------
    paper 0:ink 0
    IF tipo=1 THEN
      print at yesq,xesq;"   "
      print at yesq+1,xesq;"   "
      print at yesq+2,xesq;"   "
    END IF  
    IF tipo=2 THEN
      print at yesq,xesq;"   "
      print at yesq+1,xesq;"   "
      print at yesq+2,xesq;"   "
      print at yesq+3,xesq;"   "
      print at yesq+4,xesq;"   "
      print at yesq+5,xesq;"   "
    END IF
    IF tipo=3 THEN
      print at yesq,xesq;"   "
      print at yesq+1,xesq;"   "
...
sub coloca(yc,xc) :rem ------------- pone barril roto, llave o magia al romper un barril
...
sub mipause(frames) :REM ------ hace una pausa ---------------------------------------------------------------------------------------------
